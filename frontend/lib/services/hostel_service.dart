...
class HostelService {
  ...
  Future<void> submitHostelApplication({
    ...
    try {
      ...
      // Assign queue number (only on first submission — preserve existing number on re-submit)
      final docRef = _firestore.collection('hostelApplications').doc(applicationId);
      final existing = await docRef.get();
      final hasQueue = existing.exists &&
          existing.data()?['queueNumber'] != null &&
          (existing.data()!['queueNumber'] as num) > 0;

      int queueNumber = 0;
      if (!hasQueue) {
        final countSnap = await _firestore.collection('hostelApplications').count().get();
        queueNumber = (countSnap.count ?? 0) + 1;
      } else {
        queueNumber = (existing.data()!['queueNumber'] as num).toInt();
      }

      await docRef.set({...applicationData, 'queueNumber': queueNumber}, SetOptions(merge: true));

      debugPrint('[HostelService] Application submitted successfully with queue number.');
    } catch (e, stackTrace) {
  ...
  Future<void> generateFeeChallan({
    ...
    try {
      ...
      final studentEmail = studentData['email'] ?? application.studentId;
      // Use roll number from application as source of truth
  ...
  Stream<List<HostelModel>> getHostelsForStudent(
    ...
    try {
      // Map year to hall name based on rules
      String? preferredHallName;
      if (year == '2024' || year == '2023') {
        preferredHallName = 'Jinnah';
      } else if (year == '2022' || year == '2021') {
        preferredHallName = 'Iqbal';
      }

      // Find halls that include this year
      // Get all halls and filter in memory to avoid index requirement
      final allHallsSnap = await _firestore.collection('halls').get();
      final hallsSnap = allHallsSnap.docs.where((doc) {
        final data = doc.data();
        final assignedYears = data['assignedYears'] as List<dynamic>? ?? [];
      ...
      if (hostelIds.isNotEmpty) {
        ...
        // Check if preferred hall is full and redirect if needed
        if (preferredHallName != null && results.isNotEmpty) {
          // Check availability for preferred hall (case-insensitive)
          final preferredHostelName =
              preferredHallName; // Safe due to null check above
          final preferredHostel = results.firstWhere(
            (h) =>
                (hallMap[h.id] ?? '').toLowerCase() ==
                preferredHostelName.toLowerCase(),
