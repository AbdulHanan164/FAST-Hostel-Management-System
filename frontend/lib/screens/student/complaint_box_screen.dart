...
class _ComplaintBoxScreenState extends ConsumerState<ComplaintBoxScreen>
  ...
  Future<void> _submitComplaint() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Error submitting complaint: $e'), backgroundColor: AppColors.error),
        );
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('📝 New Complaint', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
      ),
      body: FadeTransition(
        opacity: _fadeAnim,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          const Text('Submit a Complaint', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 16)),
                          Text('We value your feedback', style: TextStyle(color: Colors.white.withValues(alpha: 0.80), fontSize: 12)),
                        ],
                      ),
                    ),
                  ],
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: _isLoading
                      ...
                      : const Text('📤 Submit Complaint', style: TextStyle(color: Colors.white, fontSize: 15, fontWeight: FontWeight.w700)),
                ),
              ),
            ],
          ),
    ...
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        if (filterStatus != null) {
          complaints = complaints.where((c) => (c['status'] as String?) == filterStatus).toList();
        }
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(
                  filterStatus == null ? 'No complaints yet' : 'No ${_labelForStatus(filterStatus!)} complaints',
                  style: const TextStyle(color: AppColors.textSecondary, fontSize: 16, fontWeight: FontWeight.w600),
                ),
                const SizedBox(height: 8),
                const Text('Tap + to submit a complaint', style: TextStyle(color: AppColors.textSecondary)),
              ],
            ),
          );
        }
          ...
          children: [
            Row(
              children: [
                ...
                StatusBadge(label: _statusLabel(status), type: _statusType(status)),
              ],
            ),
                  ...
                  children: [
                    const Row(
                      children: [
                        ...
                        SizedBox(width: 6),
                        Text('Admin Reply', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: AppColors.info)),
                      ],
                    ),
                    const SizedBox(height: 6),
                    Text(adminReply, style: const TextStyle(fontSize: 13, color: AppColors.textPrimary)),
                  ],
                ),
              ),
            ],
