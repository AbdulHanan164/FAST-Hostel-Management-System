            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate.fixed([
                  ...
                  const SizedBox(height: 16),
                  _QuickStatChips(userId: widget.user.uid),
                  const SizedBox(height: 20),
                  _AccommodationCta(userId: widget.user.uid),
                  const SizedBox(height: 24),
                  const _SectionTitle('Quick Access 🚀'),
                  const SizedBox(height: 12),
                  _MainModulesGrid(user: widget.user),
                  const SizedBox(height: 24),
            ...
            return StreamBuilder<List<Map<String, dynamic>>>(
              ...
              builder: (context, cmpSnap) {
                ...
                return SingleChildScrollView(
                  ...
                  child: Row(
                    children: [
                      _QuickChip(icon: Icons.bed_outlined, label: 'Room', value: roomLabel, color: AppColors.primary),
                      const SizedBox(width: 10),
                      const _QuickChip(icon: Icons.restaurant, label: 'Mess', value: 'Active', color: AppColors.success),
                      const SizedBox(width: 10),
                      _QuickChip(
...
class _QuickChip extends StatelessWidget {
  const _QuickChip({required this.icon, required this.label, required this.value, required this.color});
  final IconData icon;
  final String label;
  final String value;
  final Color color;
...
class _ApplicationStatusCard extends StatelessWidget {
  ...
  static const _steps = ['Applied', 'Fee Generated', 'Fee Confirmed', 'Room Assigned'];
  static const _stepStatuses = ['pending', 'fee_challan_generated', 'fee_confirmed', 'room_assigned'];
          ...
          Row(
            children: [
              ...
              StatusBadge(
                ...
                type: _currentStep >= 2 ? StatusType.success : _currentStep == 1 ? StatusType.warning : StatusType.info,
              ),
            ],
          ),
          const SizedBox(height: 16),
            ...
            children: List.generate(_steps.length * 2 - 1, (i) {
              if (i.isOdd) {
                ...
                return Expanded(
                  child: Container(
                    height: 2,
                    color: stepIdx < _currentStep ? AppColors.success : AppColors.border,
                  ),
                );
              }
              final stepIdx = i ~/ 2;
...
class _ModuleItem {
  const _ModuleItem({required this.title, required this.emoji, required this.icon, required this.color, required this.route});
  final String title;
  final String emoji;
  final IconData icon;
  final Color color;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        ...
        final isApproved = ['approved', 'fee_confirmed', 'fee_challan_generated', 'room_assigned']
        ...
        return _quickGrid(context, [
          const _ModuleItem(title: 'Mess\nSchedule', emoji: '🍽️', icon: Icons.restaurant_menu, color: Color(0xFF7C3AED), route: '/student/dashboard/mess'),
          const _ModuleItem(title: 'Mess\nAttendance', emoji: '🍴', icon: Icons.restaurant, color: AppColors.success, route: '/student/dashboard/mess-attendance'),
          const _ModuleItem(title: 'Gym\nRegistration', emoji: '🏋️', icon: Icons.fitness_center, color: AppColors.error, route: '/student/dashboard/gym-registration'),
          const _ModuleItem(title: 'Complaint\nBox', emoji: '📝', icon: Icons.feedback, color: AppColors.warning, route: '/student/dashboard/complaint-box'),
          const _ModuleItem(title: 'Notice\nBoard', emoji: '📢', icon: Icons.announcement, color: Color(0xFF0D9488), route: '/student/dashboard/notice-board'),
          const _ModuleItem(title: 'Fee\nPayments', emoji: '💰', icon: Icons.payment, color: Colors.blue, route: '/student/dashboard/payments'),
  ...
  Widget _quickGrid(BuildContext context, List<_ModuleItem> modules) {
    return GridView.count(
      ...
      children: modules
          .asMap()
          .entries
          .map((e) => _AnimatedModuleCard(item: e.value, index: e.key))
          .toList(),
    );
  }
}
...
class _AnimatedModuleCard extends StatefulWidget {
  const _AnimatedModuleCard({required this.item, required this.index});
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text('Error loading dashboard', style: Theme.of(context).textTheme.titleMedium),
          const SizedBox(height: 8),
